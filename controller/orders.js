...
class OrdersController {
  ...
  // --- Хэрэглэгчийн сагсаас checkout хийх ---
  async checkoutFromCart(req, res, next) {
    ...
    try {
      ...
      const total_amount =
        subtotal_amount + shipping_amount + Number(tax_amount) - Number(discount_amount);
      ...
      // 6) Сагсны бараа бүрийг order_items-д оруулах
      for (const item of cartItems) {
        await client.query(
          `
  ...
  async getOrdersByStatus(req, res, next) {
    try {
      const status = req.params.status; // /orders/status/:status
      const page = parseInt(req.query.page) || 0;
      const limit = parseInt(req.query.limit) || 10;

      // одоо байгаа getOrders ашиглаж болно эсвэл шууд pool-аар query
